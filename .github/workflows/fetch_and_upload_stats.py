...
def fetch_num_closed_prs_yesterday():
  ...
  url_closed_prs_this_week = f"{GITHUB_API_URL}/repos/aws-actions/{REPO_NAME}/issues?state=closed&sort=updated&direction=desc&q=is:pr+closed&since={start_date.strftime('%Y-%m-%d')}&until={end_date.strftime('%Y-%m-%d')}"
